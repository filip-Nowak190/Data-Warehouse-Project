...
Script Purpose:
    ...
    into the 'silver' schema. It performs the cleansing, standardization, 
    and business logic transformations required before the data is analysis-ready.

    Actions Performed:
        - Truncates the Silver tables before loading (full refresh each run).
        - Deduplicates customer records, keeping the most recent entry per customer.
        - Standardizes coded values (e.g. gender, marital status, product line, 
          country codes) into readable, consistent labels.
        - Splits and cleans composite keys (e.g. product key into category ID 
          and product key).
        - Validates and corrects invalid or missing values (e.g. negative prices, 
          incorrect sales amounts, invalid dates).
        - Derives calculated fields where needed (e.g. product end date, 
          recalculated sales).
        - Inserts the cleansed, transformed data into the corresponding 
          Silver tables.
